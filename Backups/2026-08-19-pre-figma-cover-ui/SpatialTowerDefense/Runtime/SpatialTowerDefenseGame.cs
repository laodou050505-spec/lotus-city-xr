...
namespace PicoTowerDefense
{
    public sealed class SpatialTowerDefenseGame : MonoBehaviour
    {
        ...
        private bool _won;
        private bool _built;
        private bool _arenaPlacedForXR;
        ...
        private void Update()
        {
            ...
            if (_input.ConsumeCycleTowerDown())
            {
            ...
            if (_input.ConsumeStartWaveDown())
            {
                ExecuteAction(SpatialAction.StartWave);
            }
            if (_input.HasTrackedHead)
            {
                if (!_arenaPlacedForXR || recenterDown)
                {
                    PlaceArenaInFrontOfHead();
                }
            }
            else
            {
        ...
        public void BuildWorld()
        {
            ...
            _input.ConfigureDesktopView(
                _arenaRoot.TransformPoint(new Vector3(0f, GameDefinitions.TableHeight + 0.08f, 0f)),
                _worldScale);
            RefreshStateDisplay();
        }
        ...
        private void BuildSpatialControls()
        {
            ...
            var controlsRoot = new GameObject("Spatial Arsenal UI").transform;
            controlsRoot.gameObject.layer = 2;
            controlsRoot.SetParent(_arenaRoot, false);
            controlsRoot.localPosition = new Vector3(0f, GameDefinitions.TableHeight + 0.16f, controlsZ);
            controlsRoot.localScale = Vector3.one * 0.52f;
            AttachBillboard(controlsRoot);

            CreateActionPedestal(controlsRoot, SpatialAction.SelectArrow, TowerKind.Arrow, "MORNING BELL\n50 LIGHT", -0.54f);
            CreateActionPedestal(controlsRoot, SpatialAction.SelectCannon, TowerKind.Cannon, "SUTRA GUARD\n120 LIGHT", -0.15f);
            CreateActionPedestal(controlsRoot, SpatialAction.SelectFrost, TowerKind.Frost, "LOTUS LAMP\n90 LIGHT", 0.24f);
            ...
            waveButton.transform.SetParent(controlsRoot, false);
        ...
        private void RefreshActionVisuals()
        {
            for (int i = 0; i < _actions.Count; i++)
            {
                ...
                bool enabled = action.Action != SpatialAction.StartWave || (!_gameOver && !_waveInProgress && _waveIndex < GameDefinitions.Waves.Length);
        ...
        private void RefreshWaveBriefing()
        {
            ...
            bool visible = !_gameOver && !_waveInProgress && !_spawning &&
        ...
        private void PlaceArenaInFrontOfHead()
        {
            if (!_input.HasTrackedHead)
            {
                return;
            ...
            Vector3 forward = _input.HeadRotation * Vector3.forward;
            forward.y = 0f;
            ...
            forward.Normalize();
            Vector3 position = _input.HeadPosition + forward * (1.36f * _worldScale);
            position.y = 0.72f - GameDefinitions.TableHeight * _worldScale;
            _arenaRoot.SetPositionAndRotation(position, Quaternion.LookRotation(forward, Vector3.up));
            _arenaPlacedForXR = true;
        }
